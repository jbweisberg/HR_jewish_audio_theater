...
import { 
  Play, Pause, X, Bell, CheckCircle2, Menu, Globe, Music, Share2 
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#050A14] text-[#F5F2E8] font-sans w-full overflow-x-hidden">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
                  {recs.map(r => (
                    <div key={r.id} onClick={() => togglePlay(r)} className="cursor-pointer group">
                      <img src={r.image} className="w-full aspect-square object-cover border border-black/5 mb-4 group-hover:scale-105 transition" alt="" />
      ...
      <nav className="fixed top-0 w-full z-50 bg-[#050A14]/95 backdrop-blur-lg border-b border-[#D4AF37]/10 px-6 py-5 md:py-8">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="hidden md:flex gap-10 text-[10px] font-black uppercase tracking-widest">
            ...
            <a href="#signup" className="bg-[#D4AF37] text-black px-5 py-2 flex items-center gap-2 transition hover:bg-white"><Bell size={12} /> Alerts</a>
          </div>
      ...
      <header className="relative min-h-screen flex items-center pt-24 px-6 md:px-12 overflow-hidden text-left">
        ...
        <div className="max-w-7xl mx-auto w-full grid md:grid-cols-12 gap-10 md:gap-20 relative z-10">
          <div className="md:col-span-7 flex flex-col justify-center">
            <h2 className="text-5xl sm:text-6xl md:text-8xl lg:text-[100px] font-serif leading-[0.9] mb-8 uppercase tracking-tighter italic">{episodes[0]?.title}</h2>
            <p className="text-lg md:text-2xl font-light opacity-80 mb-12 max-w-xl italic border-l-2 border-[#D4AF37]/30 pl-6">"{episodes[0]?.desc}"</p>
            ...
            {episodes.slice(1).map((ep) => (
              <div key={ep.id} className="group cursor-pointer flex flex-col" onClick={() => togglePlay(ep)}>
                <div className="relative aspect-square overflow-hidden bg-black mb-8 shadow-xl">
                  <img src={ep.image} className="w-full h-full object-cover opacity-80 group-hover:scale-110 transition duration-1000" alt="" />
      ...
      <section id="casting" className="py-24 md:py-40 px-6 md:px-12 bg-[#050A14] border-y border-[#D4AF37]/10 text-left">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-16 md:gap-24 items-center">
          <div>
            <h2 className="text-4xl md:text-8xl font-serif uppercase tracking-tighter mb-8 italic text-[#D4AF37] leading-none">Stage Call</h2>
      ...
      <footer id="contact" className="py-24 md:py-40 px-6 bg-[#050A14] text-center border-t border-[#D4AF37]/10">
        ...
        <p className="mt-24 text-[8px] md:text-[10px] uppercase tracking-[0.4em] opacity-20 font-bold tracking-widest leading-none italic">© 2024 Heshey Riesel • Authority Production Archive</p>
      </footer>
          ...
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-6">
            ...
            <div className="w-full flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                ...
                <button onClick={() => { setActiveEp(null); setIsPlaying(false); audioRef.current?.pause(); }} className="text-white/20 p-2 transition hover:text-white"><X size={24} /></button>
              </div>
            </div>
          </div>
        </div>
