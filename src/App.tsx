...
import { 
  ...
  Mic2, CheckCircle2, Star, Menu, Globe, Music, Share2, AlertCircle, PlayCircle
} from 'lucide-react';
...
export default function App() {
  ...
  const handleTimeUpdate = () => {
    ...
    if (dur > 0 && (dur - current <= 60) && !warned) {
      setWarned(true);
  ...
  return (
    <div className="min-h-screen bg-[#050A14] text-[#F5F2E8] font-sans w-full overflow-x-hidden">
      ...
      {/* PLAYER BAR */}
      {activeEp && (
        <div className={`fixed bottom-0 left-0 right-0 border-t-2 border-[#D4AF37] px-4 md:px-8 py-6 md:py-10 z-[100] shadow-2xl transition-colors duration-1000 ${isFinalMinute ? 'bg-[#4A0E0E]' : 'bg-[#0A0F1B]'}`}>
