...
const CACHE_KEY = "jat_master_logic_v_SEMANTIC";
...
export default function App() {
  ...
  // MULTI-PROXY GATEWAY (SPEED & CORS FIX)
  useEffect(() => {
  ...
  const handleTimeUpdate = () => {
    ...
    // MOVE INTERCEPT: 15s left. Audio keeps playing!
  ...
  // THE MOVE SEMANTIC BRAIN (Flawless Sequel Detection)
  const generateSequenceMap = () => {
    const idx = episodes.findIndex(e => e.id === activeEp.id);
    
    // Clean the current title to get the "Franchise Root"
    const cleanRoot = (title: string) => {
      return title.toLowerCase().replace(/\b(part|pt|chapter|episode)\s*\d+\b/gi, '').replace(/[-:|]/g, '').trim();
    };
    ...
    // Check the episode uploaded right after this one (Index - 1)
    if (idx > 0) {
  ...
  return (
    <div className={`min-h-screen bg-[#02040A] text-[#F5F2E8] font-sans selection:bg-[#D4AF37] overflow-x-hidden ${isDimmed ? 'is-bedtime' : ''}`}>
      ...
      {showNextOverlay && (
        <div className="fixed inset-0 z-[5000] bg-black/95 backdrop-blur-md flex items-center justify-center p-4 md:p-10 animate-in slide-in-from-bottom duration-700">
           <div className="max-w-5xl w-full bg-[#F5F2E8] text-[#02040A] p-8 md:p-14 shadow-2xl border-t-[10px] border-[#D4AF37] relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-10">
                 {transitionList.map((ep, i) => (
                   <div key={ep.id} onClick={() => togglePlay(ep)} className={`cursor-pointer group p-5 border-2 transition-all duration-500 ${i === 0 && isSeriesLink ? 'bg-white border-[#D4AF37] ring-8 ring-[#D4AF37]/10 scale-105 shadow-2xl' : 'bg-black/5 border-transparent opacity-80 hover:opacity-100 grayscale hover:grayscale-0'}`}>
        ...
        {episodes.length > 0 && (
          <header className="relative min-h-screen flex items-center pt-24 px-8 text-left z-10 overflow-hidden mb-32">
            ...
            <div className="max-w-7xl mx-auto w-full grid md:grid-cols-12 gap-10 md:gap-24 relative z-10 items-center">
              ...
              <div className="hidden md:block md:col-span-5"><img src={episodes[0].image} className="w-full aspect-square object-cover border-8 border-[#D4AF37]/10 shadow-[0_0_100px_#000] grayscale transition duration-1000" /></div>
            </div>
          </header>
        )}
            ...
            <div>
              ...
              <div className="flex items-center gap-6 text-[#D4AF37] font-black uppercase tracking-widest text-xs">
                <span className="flex items-center gap-2 text-white bg-[#4A0E0E] px-4 py-2">
                  <Star size={16} fill="#D4AF37"/> Open Auditions
                </span>
              </div>
            </div>
      ...
      {/* --- MASTER PLAYER BAR --- */}
      {activeEp && (
        <div className={`fixed bottom-0 left-0 right-0 border-t-2 border-[#D4AF37]/50 px-6 md:px-12 py-10 md:py-16 z-[3000] shadow-[0_-30px_150px_#000] transition-colors duration-1000 ${duration - currentTime <= 60 && !showNextOverlay ? 'bg-[#7B0000]' : 'bg-[#090D17]'}`}>
          <div className="max-w-7xl mx-auto text-left">
            ...
            <div className="w-full flex justify-between items-center gap-10">
              <div className="flex items-center gap-8 text-left truncate flex-1 cursor-pointer" onClick={() => window.scrollTo({top:0, behavior:'smooth'})}>
              ...
              <div className="flex items-center gap-6 md:gap-14">
                <button onClick={() => setIsDimmed(!isDimmed)} className={`hidden sm:block p-4 md:p-6 rounded-full border transition-all ${isDimmed ? 'bg-[#D4AF37] text-black border-[#D4AF37] shadow-[0_0_50px_#D4AF3744]' : 'bg-white/5 text-white/20 border-white/5 hover:border-[#D4AF37]'}`}>
                   <Lamp size={32}/>
                </button>
